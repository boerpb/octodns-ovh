...
class TestOvhProvider(TestCase):
    ...
    valid_dkim_key = (
        "p=MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCxLaG16G4SaE"
    ...
    expected.add(
        Record.new(
            zone, '4', {'ttl': 900, 'type': 'PTR', 'value': 'unit.tests.'}
        )
    )
    ...
    expected.add(
        Record.new(
            zone, 'dkim', {'ttl': 1300, 'type': 'TXT', 'value': valid_dkim_key}
        )
    )
    ...
    expected.add(
        Record.new(
            zone, 'txt', {'ttl': 1400, 'type': 'TXT', 'value': 'TXT text'}
        )
    )
